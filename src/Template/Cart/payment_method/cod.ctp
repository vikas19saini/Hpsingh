...
<div class="payment_details">
    <p>A COD charge of <price><strong><?= $this->Number->currency($cod_charges, $defaultCurrency->code) ?></strong></price> will be applicable. Total payable amount is <price><strong><?= $this->Number->currency($total_amount, $defaultCurrency->code) ?></strong></price>
    </p>
    ...
    <script src="https://www.google.com/recaptcha/api.js">
        grecaptcha.ready(function() {
            grecaptcha.execute('<?= env('GOOGLE_CAPTCHA_KEY') ?>', {
                action: 'homepage'
            }).then(function(token) {});
        });
    </script>
